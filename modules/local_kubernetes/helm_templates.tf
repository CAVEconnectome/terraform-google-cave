resource "local_file" "helm_values_cluster" {
  ...
  content = templatefile("${path.module}/templates/cluster.tpl", {
    ...
    standard_pool_name    = var.standard_pool_name,
    lightweight_pool_name = var.lightweight_pool_name,
    mesh_pool_name        = var.mesh_pool_name,
    docker_registry       = var.docker_registry,
    data_project_id       = var.bigtable_google_project != "" ? var.bigtable_google_project : var.project_id,
    dns_entries           = [for _, v in var.dns_entries : v.domain_name]
  })
}
...
resource "local_file" "bootstrap_configure_script" {
  ...
  content = templatefile("${path.module}/templates/configure.sh.tpl", {
    ...
    kube_context = format("gke_%s_%s_%s", var.project_id, var.zone, var.cluster_name)
  })
